# -*- coding: utf-8 -*-
"""Caspian & Black Sea Monitor - Data Extraction Pipeline"""
import os
import json
import yaml
import requests
...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            ...
            "sources": ["aisstream.io AIS", "NASA FIRMS", "Google News RSS", "Copernicus Sentinel"],
            "version": "1.0.0"
        },
        "stats": [
            {"label": "Vessels Tracked", "value": "1,247", "delta": "+5%"},
            {"label": "Active Routes", "value": "89", "delta": "+2"},
            {"label": "Port Calls (24h)", "value": "34", "delta": "-3%"},
            {"label": "Anomalies", "value": "3", "delta": "stable"},
        ],
        "entities": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "aisstream.io AIS"}
            for i in range(1, 16)
        ],
        "timeseries": [
